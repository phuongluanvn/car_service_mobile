        ...
        if (state.detailStatus == ProcessDetailStatus.success) {
          return BlocBuilder<CustomerServiceBloc, CustomerServiceState>(
            ...
            builder: (context, sstate) {
              if (sstate.status == CustomerServiceStatus.init) {
                return CircularProgressIndicator();
              } else if (sstate.status == CustomerServiceStatus.loading) {
                return CircularProgressIndicator();
                    ...
                    BlocListener<UpdateItemBloc, UpdateItemState>(
                      listener: (context, state) {
                        if (state.deleteStatus ==
                          ...
                          processBloc.add(
                              DoProcessOrderDetailEvent(email: widget.orderId));
                        }
                      },
                      child: ElevatedButton(
                        ...
                        onPressed: () {
                          updateItemBloc.add(DeleteServiceByIdEvent(
                              detailId: state.processDetail[0]
                                  .orderDetails[widget.index].id));
                        },
                      ),
                    ),
                  ],
                        ...
                        title: Row(
                          ...
                          children: [
                            Text(state.processDetail[0]
                                .orderDetails[widget.index].name),
                            Text(
                            ...
                            child: BlocBuilder<AccessoryBloc, AccessoryState>(
                              ...
                              builder:
                                ...
                                if (accstate.status ==
                                  ...
                                  for (int i = 0;
                                      i < state.processList.length;
                                      i++) {
                                    // _priceAcc = accstate.accessoryList
                                    //             .indexWhere((element) =>
                                    //                 element.id ==
                                    //                 state
                                    //                     .processList[0]
              ...
              } else if (sstate.status == CustomerServiceStatus.error) {
                return ErrorWidget(sstate.message.toString());
              }
              ;
            },
          );
